...
import { useGetProforma } from "@workspace/api-client-react";
...
export default function ProformaDetail() {
  ...
  if (!proforma) return <div>Proforma introuvable</div>;
          ...
          <CardContent>
            ...
            <div className="mt-6 flex justify-end">
              <div className="w-64 space-y-2">
                <div className="flex justify-between text-sm"><span>Sous-total</span><span>{formatCurrency(proforma.subtotal)}</span></div>
                <div className="flex justify-between text-sm"><span>TVA</span><span>{formatCurrency(proforma.taxAmount)}</span></div>
                <div className="flex justify-between font-bold text-lg pt-2 border-t"><span>Total</span><span>{formatCurrency(proforma.total)}</span></div>
              </div>
            </div>
          </CardContent>
        </Card>
